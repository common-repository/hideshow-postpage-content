...
use \SOSIDEE_HSC\SOS\WP as SOS_WP_ROOT;
...
class WpDatabase
{
    ...
    public function create() {
        add_action( 'plugins_loaded', function() {
            ...
            if ( empty($current) ) {
                $plugin::msgErr("Plugin version is empty. Check if initialize() function contains the line 'parent::initialize();'", true);
            }
            ...
            if ( version_compare($installed, $current) < 0 ) {
                if ( $this->createTables() ) {
                    ...
                    $plugin->onDatabaseUpdate();
                } else {
                    if ( is_admin() ) {
                        for ( $n=0; $n<count($this->lastErrors); $n++ ) {
                            $plugin::msgErr( $this->lastErrors[$n], true );
                        }
                    }
                }
            }
